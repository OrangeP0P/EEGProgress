...
# channel_location = pd.read_excel('channel_location_R.xls')
...
file_handle.write('程序简述：PSDAN[调参]【不使用任何loss，测试150epoch的准确率】\n '
                  '（1）数据进行横向重排，左右脑数据分开输入网络，电极数55×2\n '
                  '（2）程序部署于：30780')
...
for seed in range(EX, EX + 1):  # 随机五个随机种子数
        ...
        for k in range(0, len(subject_list)):
            for g in range(0, len(subject_list)):
                if subject_list[k] != subject_list[g] and Start_Task <= Task <= Terminal_Task:
                    ...
                    # input_target = torch.from_numpy(train_source_data.astype(np.float32))
                    # flops, t_params = profile(net[0], inputs=(input_source, input_target,), verbose=False)
                    ...
                    for epoch in range(args.epochs):
                        ...
                        Max_Point = 300
                        if epoch > 50:
                            la_1 = 2 / (1 + math.exp(-10 * ((epoch - 50) / Max_Point))) - 1
                        else:
                            la_1 = 0
                        if epoch > 50:
                            la_2 = 1 / (1 + math.exp(10 * ((epoch - 50) / Epoch))) + 0.5
                        else:
                            la_2 = 1
                        if epoch > 150:
                            la_3 = 2 / (1 + math.exp(-10 * ((epoch - 150) / Max_Point))) - 1
                        else:
                            la_3 = 0
                        if epoch > 200:
                            la_4 = 2 / (1 + math.exp(-10 * ((epoch - 200) / Max_Point))) - 1
                        else:
                            la_4 = 0
                        ...
                        for i in range(0, num_iter):
                            train_source_data_L1, train_source_label_1 = iter_source_L.next()
                            train_target_data_L1, _ = iter_target_L.next()

                            train_source_data_R1, _ = iter_source_R.next()
                            train_target_data_R1, _ = iter_target_R.next()
                            ...
                            for key, net in net_dict.items():
                                ...
                                output, intra_location_loss, inter_location_loss_LR, inter_location_loss_ST, inter_global_loss = \
                                    net[0](train_source_data_L1, train_target_data_L1, train_source_data_R1, train_target_data_R1)
                                clf_loss = net[2](output, train_source_label_1.long())
                                tr_loss = l_cls * clf_loss + la_1 * l_intra * intra_location_loss + l_inter_LR / (la_2 * inter_location_loss_LR) + \
                                          la_3 * l_inter_ST * inter_location_loss_ST + la_4 * l_global * inter_global_loss
                                net[1].zero_grad()
                                tr_loss.backward()
                                net[1].step()
                                net[3].step()
                        ...
                        list.append(GLOBAL_LOSS, inter_global_loss.item())
                        list.append(INTRA_LOCATION_LOSS, intra_location_loss.item())
                        list.append(INTER_LOCATION_LOSS_LR, inter_location_loss_LR.item())
                        list.append(INTER_LOCATION_LOSS_ST, inter_location_loss_ST.item())
                        ...
                        for validation_data_L, validation_data_R, validation_label in validation_loader:
                            ...
                            va_pred, va_global_loss, va_intra_ele_loss, va_intra_ele_loss, va_inter_ele_loss = \
                                net[0](validation_data_L, validation_data_L, validation_data_R, validation_data_R)
                        ...
                        # # 保存每个epoch的模型
                        # torch.save(net[0], 'Transfer_Model/Task-' + str(Task) +
                        #            '/' + 'Cross-' + str(Cross_Mission + 1) +
...
print('Seed average acc = ', acc)
file_handle.write('Seed average acc = ' + str(acc) + '\n')
file_handle.close()
