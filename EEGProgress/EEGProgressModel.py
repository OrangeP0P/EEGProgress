...
class EEGProgress_TypeI(nn.Module):  # New architecture
    def __init__(self, act_func):
        super(EEGProgress_TypeI, self).__init__()
        self.F1 = 8
...
class EEGProgress_TypeII(nn.Module):
    def __init__(self, act_func):
        super(EEGProgress_TypeII, self).__init__()
        self.F1 = 8

        self.TEMP_Sequence1 = nn.Sequential()
        self.TEMP_Sequence1.add_module('T1-Conv1', nn.Conv2d(1, self.F1, (1, 25), stride=(1, 1)))
        self.TEMP_Sequence1.add_module('T1-Norm1', nn.BatchNorm2d(self.F1, False))
        self.TEMP_Sequence1.add_module('T1-ELU', nn.ReLU())
        self.TEMP_Sequence1.add_module('T1-AVGPool1', nn.AvgPool2d((1, 5), stride=(1, 5)))
        self.TEMP_Sequence1.add_module('T1-Drop1', nn.Dropout(p=0.25))

        self.ELE_Sequence0 = nn.Sequential()
        self.ELE_Sequence0.add_module('E0-ZeroPad1', nn.ZeroPad2d(padding=(0, 0, 1, 1)))
        self.ELE_Sequence0.add_module('E0-Conv1', nn.Conv2d(self.F1, self.F1 * 2, (3, 1), stride=(1, 1)))
        self.ELE_Sequence0.add_module('E0-Norm1', nn.BatchNorm2d(self.F1 * 2, False))
        self.ELE_Sequence0.add_module('E0-Drop1', nn.Dropout(p=0.25))

        self.TEMP_Sequence2 = nn.Sequential()
        self.TEMP_Sequence2.add_module('B-Conv1', nn.Conv2d(self.F1 * 2, self.F1 * 2, (1, 10), stride=(1, 1)))
        self.TEMP_Sequence2.add_module('B-Norm1', nn.BatchNorm2d(self.F1 * 2, False))
        self.TEMP_Sequence2.add_module('B-ELU1', nn.ReLU())
        self.TEMP_Sequence2.add_module('B-AVGPool1', nn.AvgPool2d((1, 5), stride=(1, 5)))
        self.TEMP_Sequence2.add_module('B-Drop1', nn.Dropout(p=0.25))

        self.Electrode_Conv = nn.Sequential()
        self.Electrode_Conv.add_module('E1-Conv1', nn.Conv2d(self.F1 * 2, self.F1 * 2, (15, 1), stride=(10, 1)))
        self.Electrode_Conv.add_module('E1-Norm1', nn.BatchNorm2d(self.F1 * 2, False))

        self.Region_Conv = nn.Sequential()
        self.Region_Conv.add_module('E2-Conv1', nn.Conv2d(self.F1 * 2, self.F1 * 2, (5, 1), stride=(5, 1)))
        self.Region_Conv.add_module('E2-Norm1', nn.BatchNorm2d(self.F1 * 2, False))

        self.Hemisphere_Conv = nn.Sequential()
        self.Hemisphere_Conv.add_module('E2-Conv1', nn.Conv2d(self.F1 * 2, self.F1 * 2, (2, 1), stride=(2, 1)))
        self.Hemisphere_Conv.add_module('E2-Norm1', nn.BatchNorm2d(self.F1 * 2, False))
        self.Hemisphere_Conv.add_module('E2-ELU1', nn.ReLU())
        self.Hemisphere_Conv.add_module('E2-Drop1', nn.Dropout(p=0.25))

        self.FC = nn.Sequential()
        self.FC.add_module('E_FC1', nn.Linear(176, 2))
    ...
    def forward(self, source_L, source_R):
        ...
        source_L = self.ELE_Sequence0(source_L)
        source_R = self.ELE_Sequence0(source_R)

        source_L = self.TEMP_Sequence2(source_L)
        source_R = self.TEMP_Sequence2(source_R)

        source_ele_L = self.Electrode_Conv(source_L)
        source_ele_R = self.Electrode_Conv(source_R)

        source_ele_L = self.Region_Conv(source_ele_L)
        source_ele_R = self.Region_Conv(source_ele_R)

        source_LR = torch.cat((source_ele_L, source_ele_R), -2)  # 拼接左右脑数据

        source_LR = self.Hemisphere_Conv(source_LR)

        # 拉伸成条形处理
        s0, s1, _, s2 = source_LR.shape[:4]  # 读取张量大小
        source_LR = source_LR.reshape(s0, s1 * s2)  # [28,118*2]
        output = self.FC(source_LR)
        return output
...
'''选取网络和激活函数'''
def choose_net(args):
    if args.model == 'EEGProgress_TypeI':
        return {
        'elu': [EEGProgress_TypeI('relu')]
        }
    if args.model == 'EEGProgress_TypeII':
        return {
        'elu': [EEGProgress_TypeII('relu')]
        }
    else:
